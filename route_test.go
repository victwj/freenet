...
func TestGetRouteMatch(t *testing.T) {
	...
	result = n.getRoutingTableEntry("aec", 1) // should be either n1 or n3
	// Match n1 since it was added first
	if result != n1 {
		t.Error(result)
	}

	result = n.getRoutingTableEntry("aec", 2) // should be either n1 or n3
	if result != n3 {
		t.Error(result)
	}

	result = n.getRoutingTableEntry("aec", 3) // should be n2
	if result != n2 {
		t.Error(result)
	}

	result = n.getRoutingTableEntry("aec", 5) // should be nil
	if result != nil {
		t.Error(result)
	}
}
